...
<tr><td><b>Swappable engines <em>(QweenBee · Solo)</em></b></td><td>AdaptOrch isn't the only way to run a turn — pick the engine that fits the work with <b><code>/settings</code> → orchestration engine</b> (or <code>orchestration.engine</code> in <code>anet.config.yaml</code>).<br><b>QweenBee</b> replaces fixed topologies with an LLM planner that generates a <b>temporal communication graph</b> per task — who messages whom, in which round — executed directly by a temporal executor. A skill bank distills Preserve/Modify/Avoid design rules from prior runs (conservative credit + a held-out acceptance gate before anything's trusted), and a built-in bench (<code>python -m anet.QweenBee.bench</code>) A/B-tests it against AdaptOrch on identical tasks.<br><b>Solo</b> skips orchestration entirely: one generalist agent, a curated toolset, no decomposition step — the cheapest option per turn, for when a request doesn't need a committee. It still delegates on demand via <code>spawn_tool</code>.</td></tr>
...
<tr><td><b>Trims its own context <em>(AgentDiet)</em></b></td><td>Every tool result an agent collects is resent to the model on <em>every</em> later step — so one big file read early in a long task gets billed dozens of times. When enabled, a cheap model rewrites each step once the agent has moved past it, replacing stale output with a short note (<code>[… 2,890 more lines omitted …]</code>) while keeping what still matters. Measured here at <b>47% fewer input tokens</b> on a long tool-loop task, in line with the <a href="references/Reducing%20Cost%20of%20LLM%20Agents%20with%20Trajectory%20Reduction-with-annotations.pdf">paper's</a> 40–60%. Works in <em>all three</em> engines. Off by default — turn it on with a <code>diet:</code> block in <code>anet.config.yaml</code>.</td></tr>
...
```yaml
orchestration:
  engine: adaptorch     # adaptorch (default) · qweenbee · solo — or use /settings

# Trim stale tool output from long tool-loops (all engines). Off by default.
diet:
  enabled: true
  model: openai/gpt-5.4-mini   # use a CHEAP model; defaults to the manager model
```

Pick the engine by the work: **adaptorch** for multi-domain requests worth decomposing, **qweenbee** to try planner-generated communication graphs, **solo** when you want one agent and the lowest per-turn cost. `diet` is orthogonal — it pays off on long tool-heavy tasks (a `code_agent` working through a repo) and stays dormant on short chats. Its cost shows up as a `diet` stage in the token footer.
