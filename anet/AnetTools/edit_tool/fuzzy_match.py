"""
fuzzy_match.py — multi-strategy find-and-replace for edit_tool.

Robustly locates old_string in a file even when the model's copy has small
whitespace, indentation, escape, or unicode differences from the file on disk —
the single most common reason a surgical edit fails.

The 9-strategy chain (ported from the hermes-agent project, OpenCode-inspired),
tried in order from strict to permissive:
  ...
  5. escape_normalized     — turn literal \\n \\t \\r into real chars
  6. trimmed_boundary      — trim only the first/last line of the block
  7. unicode_normalized    — smart quotes, dashes, ellipsis, nbsp → ASCII
  8. block_anchor          — match first+last line, score the middle by similarity
  9. context_aware         — 50% of lines must be ≥80% similar

Multi-occurrence matches are governed by replace_all.

Usage:
    from anet.AnetTools.edit_tool.fuzzy_match import fuzzy_find_and_replace
...
def fuzzy_find_and_replace(
    ...
    Returns (new_content, match_count, strategy_name, error_message):
      - success: (modified_content, n_replacements, strategy_used, None)
      - failure: (original_content, 0, None, error_description)
    """
    if not old_string:
    ...
    for strategy_name, strategy_fn in strategies:
        ...
        # Escape-drift guard: when a non-exact strategy matched, and new_string
        # carries shell/JSON-style \\' or \\" that the matched file region lacks,
        # writing it verbatim would corrupt the file. Almost always tool-call
        # serialization drift — block it so the model re-reads and retries.
...
def _detect_escape_drift(
    ...
    """Detect tool-call escape-drift artifacts (\\' or \\") in new_string."""
    ...
    for suspect in ("\\'", '\\"'):
        if suspect in new_string and suspect in old_string and suspect not in matched_regions:
            ...
            return (
                ...
                f"literal sequence {suspect!r} but the matched region of the file "
                f"does not. This is almost always a tool-call serialization "
                f"artifact where an apostrophe or quote got prefixed with a "
                f"spurious backslash. Re-read the file and pass old_string/"
...
def _build_orig_to_norm_map(original: str) -> List[int]:
    """Map each original char index to its index in the unicode-normalized string.

    UNICODE_MAP replacements can expand a char (em-dash → '--'), so the normalized
    string may be longer; this lets us map normalized positions back to original.
    """
    result: List[int] = []
    norm_pos = 0
...
def _strategy_context_aware(content: str, pattern: str) -> List[Tuple[int, int]]:
    ...
    # Guard: this is the only strategy that matches on per-line *similarity*
    # rather than exact (post-normalization) line equality. On short patterns
    # the surrounding context is too thin — e.g. "def calculate_sum(items):"
    # is >80% similar to "def calculate_total(items):" and would be silently
    # edited as the wrong line. A failed edit is recoverable; a wrong one is
    # not. Require ≥3 lines so multi-line context disambiguates the block.
...
def _map_normalized_positions(
    ...
    """Best-effort map of whitespace-normalized positions back to original."""
...
def find_closest_lines(
    ...
    """Return a 'did you mean?' snippet of the closest lines to old_string."""
...
def format_no_match_hint(
    ...
    """Append a 'Did you mean...' snippet, but only for genuine no-match errors."""
    if match_count != 0:
        return ""
