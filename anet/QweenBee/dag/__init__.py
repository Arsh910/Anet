"""
anet.QweenBee.dag — Phase 2 of the QweenBee pipeline: static DAG construction.

Pure Python, no LLM calls, no external deps. Turns a list of decomposed subtasks
into a formal task dependency DAG G_T = (V, E, w, c) and computes structural
properties:
    ...
    layers     topological layering — the planner's deterministic fallback
               (anet.QweenBee.planner._fallback_graph) maps these directly to
               temporal-graph rounds

All algorithms run in O(|V| + |E|) (Kahn layering + a single DP pass), matching
the paper's §4.3 complexity. ω uses the layer-width approximation.

This module is the foundation: the decomposer (Phase 1) produces Subtasks,
build() turns them into a TaskDAG, and the planner (Phase 3) reads it — to
generate a temporal communication DAG directly, and as the source of its
