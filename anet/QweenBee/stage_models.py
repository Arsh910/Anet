"""
stage_models.py — per-stage LLM model resolution for the QweenBee pipeline.

Each orchestration stage that needs a model (decomposer, planner) may declare
its own model/provider under an `orchestration:` block in anet.config.yaml. If
a stage isn't configured, it falls back to the manager's model/provider — so
by default everything uses the manager model, and you only override a stage
when you want to (e.g. a cheap model for decomposition):

    orchestration:
      decomposer: { model: gemini-2.5-flash,  provider: google }
      planner:    { model: claude-sonnet-4.6, provider: openrouter }

Stages with no model needs (dag, tgraph) never call this. Shared so every
stage resolves its model the same way.
"""
