...
MCPSMITH_SYSTEM_PROMPT = (
    ...
    "4b. WRITE mcps/<name>/README.md (file_tool write_file) — this is what makes the server\n"
    "    SHAREABLE. PackSmith reads it to write setup steps for a recipient. Use this shape:\n"
    "      # <name> MCP\n"
    "      Source: package | repo | local   (package = npx/uvx/published; repo = clone+build;\n"
    "                                         local = personal/unpublished)\n"
    "      Repo: <git url, if any>\n"
    "      Install: <how to obtain it — e.g. 'npm install -g <pkg>', or clone+build commands>\n"
    "      Entry: <what command/args should point at on the user's machine>\n"
    "      Env: <KEY names it needs, or 'none'>\n"
    "      <one paragraph: what the server does>\n"
    "    Set 'Source: package' when you used npx/uvx (auto-fetches — recipient needs only the\n"
    "    runtime). Set 'Source: repo' for a cloned server (its CODE IS STRIPPED on export, so\n"
    "    the Repo/Install/Entry lines are how the recipient rebuilds it). Base every line on the\n"
    "    real docs; if you don't know the repo URL, write 'Repo: unknown — recipient must locate'.\n"
