...
async def run(params: dict) -> dict:
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        if file_path:
            ...
            # Try the detected method; fall back to sendDocument if Telegram rejects it
            ...
            if not result.get("ok") and method != "sendDocument":
                # Telegram couldn't process the file as a photo/video/audio — send as raw document
                with p.open("rb") as fh:
                    resp = await client.post(
                        f"{base}/sendDocument",
                        data=form,
